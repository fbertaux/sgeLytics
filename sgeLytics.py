...
class SgeModel(object):
    ...
    def defineModelFromCVTau_rg_rm_rp(self, CV, Tau, rg, rm, rp):
        ...
        if Tau <= Tau_min or Tau >= Tau_max:
            raise Exception('Tau constraint not compatible with rg, rm, rp '+\
                            '(Tau_min = %f, Tau_max = %f)' % (Tau_min, Tau_max))
    ...
    def defineModelFromCVTauHLP(self, CV, Tau, HLP, desired_HLM=None):
        (rg, EG, EM, HLM, best_Tau) = \
                find_params_given_CV_Tau_HLP(CV, Tau, HLP, desired_HLM)
        Toff = 1. / rg / EG
        Ton = Toff * EG / (1.-EG)
        self.defineModel(Ton, Toff, EM, HLM, HLP)
    def defineModelFromCVTauHLM(self, CV, Tau, HLM, desired_HLP=None):
        (rg, EG, EM, HLP, best_Tau) = \
                find_params_given_CV_Tau_HLM(CV, Tau, HLM, desired_HLP)
        Toff = 1. / rg / EG
        Ton = Toff * EG / (1.-EG)
        self.defineModel(Ton, Toff, EM, HLM, HLP)
    def giveCV(self):
        return computeCV(rg=self.rg, rm=self.rm, rp=self.rp, EG=self.EG, EM=self.EM)
    def giveAutocProt(self, t):
        return computeAutocorrelationProt(rg=self.rg, rm=self.rm, rp=self.rp,
                                          EG=self.EG, EM=self.EM, t=t)
    def giveMixingTime(self, autoc=0.5):
        return estimateTau(rg=self.rg, rm=self.rm, rp=self.rp, EG=self.EG,
                           EM=self.EM, autoc=autoc)


### direct, explicit analytical expressions
def computeF(rg, rm, rp):
    return rp * rm * (rp+rm+rg) / (rp+rm) / (rp+rg) / (rm+rg)
def computeCV_2(rg, rm, rp, EG, EM):
    return computeF(rg=rg, rm=rm, rp=rp) * (1.-EG)/EG + rp/(rm+rp)/EM
def computeCV(rg, rm, rp, EG, EM):
    return sqrt(computeCV_2(rg=rg, rm=rm, rp=rp, EG=EG, EM=EM))
def computeCV2_M(rm, rp, EM):
    return rp / (rm+rp) / EM
def computeHG(rg, rm, rp, t):
    if (rp != rm) and (rp != rg) and (rg != rm):
        return ((rm-rg)*exp(-rp*t) + (rp-rm)*exp(-rg*t) + (rg-rp)*exp(-rm*t)) \
                * rm * rp / (rm-rg) / (rp-rg) / (rp-rm)
    else:
        if rp == rm:
            if rg == rp:
                return rp**2 * t**2 * exp(-rp*t) / 2.
            else:
                return rp**2. / (rp-rg)**2 * exp(-rp*t) * (exp((rp-rg)*t) - 1. - (rp-rg)*t)
        else:
            if rp == rg:
                return rm*rp / (rp-rm)**2 * exp(-rp*t) * (exp((rp-rm)*t) - 1. - (rp-rm)*t)
            else:
                return rp*rm / (rm-rp)**2 * exp(-rm*t) * (exp((rm-rp)*t) - 1. - (rm-rp)*t)
def computeHM(rm, rp, t):
    if rm != rp:
        return rp / (rp-rm) * (exp(-rm*t) - exp(-rp*t))
    else:
        return rm*t*exp(-rm*t)
def computeAutocorrelationProt(rg, rm, rp, EG, EM, t):
    return exp(-rp*t) + computeHM(rm, rp, t) + \
            (1.-EG) / EG / computeCV_2(rg, rm, rp, EG, EM) * \
            rp * rm / (rp+rg) / (rm+rg) * computeHG(rg, rm, rp, t)
def computeEG_from_F_CV2PG(F, CV2PG):
    return F / (F + CV2PG)
def computeEG_from_rg_rm_rp_CV2PG(rg, rm, rp, CV2PG):
    return computeEG_from_F_CV2PG(computeF(rg, rm, rp), CV2PG)
def computeEM_from_rm_rp_CV2PM(rm, rp, CV2PM):
    return rp / (rp + rm) / CV2PM

### analytical expressions under an implicit form
def costEstimationTau(t, rg, rm, rp, EG, EM, autoc):
    return (computeAutocorrelationProt(rg, rm, rp, EG, EM, t) - autoc) ** 2
def estimateTau(rg, rm, rp, EG, EM, autoc=0.5):
    result = optimize.minimize_scalar(
        costEstimationTau, args=(rg, rm, rp, EG, EM, autoc),
        method='golden', tol=None, options=None)
    return result.x
def costEstimationRG_from_F_rm_rp(rg, F, rm, rp):
    return (F - computeF(rg, rm, rp))**2. / F**2.
def estimateRG_from_F_rm_rp(F, rm, rp):
    result = optimize.minimize_scalar(
        costEstimationRG_from_F_rm_rp, args=(F, rm, rp),
        method='golden', tol=None, options=None)
    return result.x
...
def find_best_rg_for_Tau_given_CV2PG_rm_rp_EM(Tau, CV2PG, rm, rp, EM):
    result = optimize.minimize_scalar(
        cost_find_best_rg_for_Tau_given_CV2PG_rm_rp_EM, args=(Tau, CV2PG, rm, rp, EM),
        method='golden', tol=None, options=None)
    rg = result.x
    EG = computeEG_from_rg_rm_rp_CV2PG(rg, rm, rp, CV2PG)
...
def find_best_rg_EM_for_Tau_given_CV_rm_rp(Tau, CV, rm, rp):
    result = optimize.minimize_scalar(
        cost_find_rg_EM_for_Tau_given_CV_rm_rp, args=(Tau, CV, rm, rp),
...
def find_params_given_CV_Tau_HLM(CV, Tau, HLM, desired_HLP=None):
    if HLM > Tau:
        raise Exception('HLM cannot be longer than Tau for this model! ' + \
                        '(did not try to prove it but seems true)')
    if not desired_HLP:
        desired_HLP = HLM*3.
...
def estimate_alpha_from_rg_rm_rp_Tau(rg, rm, rp, Tau):
    result = optimize.minimize_scalar(
        cost_estimate_alpha_from_rg_rm_rp_Tau, args=(rg, rm, rp, Tau),
        method='bounded', bounds=(0., 1.))
    return result.x
...
def estimateRG_from_rm_rp_CV_EG_EM(rm, rp, CV, EG, EM, error_if_impossible=True):
    ...
    print(CV_2_G / CV / CV)
    if CV_2_G <= 0:
        if error_if_impossible:
            raise Exception('Impossible constraints: ' + \
                            'variance part from gene superior to requested total variance')
        else:
            return 0. # why returning 0 here ?
    F = CV_2_G * EG / (1-EG)
    if F >= 1:
        if error_if_impossible:
            raise Exception('Impossible constraints: ' + \
                            'Variance part from gene cannot be that big. Lower EG ?')
        else:
            return 0. # why returning 0 here ?
...
def estimate_EG_RG_from_EM_rm_rp_CV_Tau(EM, rm, rp, CV, Tau, error_if_impossible=True):
    result = optimize.minimize_scalar(
        costEstimation_EG_RG_from_EM_rm_rp_CV_Tau, args=(EM, rm, rp, CV, Tau),
        method='bounded', bounds=(0., 1.))
    EG = result.x
    if result.fun > 1e-5:
        if error_if_impossible:
